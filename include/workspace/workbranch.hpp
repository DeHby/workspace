...
class workbranch {
    ...
    using worker_map = std::unordered_map<worker_id, worker_entry>;
...
public:
    ...
    ~workbranch() {
        ...
        bool res;
        {
            ...
            res = thread_cv.wait_for(lock, std::chrono::seconds(1), [this] { return pending_deletions.load() == 0; });
        }
...
private:
    ...
    void mission(worker_id id) {
        ...
        lok.lock();
        worker_entry& worker = workers[id];
        lok.unlock();
